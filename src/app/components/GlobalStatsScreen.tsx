...
// GlobalStatsScreen.tsx - Full profile/stats page + Daily Leaderboard section
import React, { useMemo, useState, useEffect } from "react";
import { motion, AnimatePresence } from "motion/react";
import {
  X, Star, Trophy, Zap, Calendar, BookOpen, Timer,
  Globe, Award, BarChart3, TrendingUp, Map, Medal,
...
function loadDailyResult(): { score: number; date: string; streak: number } | null {
  try {
    ...
    const scopedKey = `dailyChallenge_result_${userId ?? "guest"}`;
    const legacyKey = "dailyChallenge_result";

    const raw = localStorage.getItem(scopedKey) || localStorage.getItem(legacyKey);
    const streakRaw = localStorage.getItem(`dailyChallenge_streak_${userId ?? "guest"}`)
      || localStorage.getItem("dailyChallenge_streak");
...
function DailyLeaderboardSection({ username }: { username: string | null }) {
  ...
  // Show top 3 collapsed, all when expanded
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-white font-black text-sm">Daily Leaderboard</p>
            <p className="text-indigo-200 text-[10px]">{date} · Today's challenge</p>
          </div>
        </div>
        {userRank && (
          <div className="bg-white/20 rounded-xl px-2.5 py-1 text-center">
            <p className="text-white font-black text-sm">#{userRank}</p>
            <p className="text-indigo-200 text-[9px]">Your rank</p>
          </div>
        )}
      </div>
      ...
      <div className="p-3">
        {loading ? (
          ...
          <p className="text-center text-xs text-slate-400 py-4">
            Couldn't load leaderboard. Play the daily challenge to appear here.
          </p>
        ) : entries.length === 0 ? (
          <p className="text-center text-xs text-slate-400 py-4">
          ...
          <>
            {/* Column headers */}
            <div className="grid grid-cols-[28px_1fr_40px_52px] gap-1 px-2 pb-1.5 border-b border-slate-100 dark:border-slate-700">
            ...
            <div className="space-y-0.5 mt-1">
              {visible.map((entry) => {
                ...
                return (
                  <div key={entry.rank}
                    className={`grid grid-cols-[28px_1fr_40px_52px] gap-1 px-2 py-1.5 rounded-xl items-center ${
                      isMe
            ...
            {/* Show user if outside visible range */}
            {!expanded && userRank !== null && userRank > 3 && username &&
              !entries.slice(0, 3).find(e => e.username === username) && (
              <div className="mt-1 pt-1 border-t border-dashed border-slate-200 dark:border-slate-600">
                <div className="grid grid-cols-[28px_1fr_40px_52px] gap-1 px-2 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-900/25 border border-indigo-200 dark:border-indigo-700/50 items-center">
                  ...
                  <span className="text-xs font-black text-center text-slate-500">—</span>
            ...
            {entries.length > 3 && (
              <button onClick={() => setExpanded(e => !e)}
                className="w-full mt-2 py-1.5 text-[11px] font-semibold text-indigo-500 dark:text-indigo-400 flex items-center justify-center gap-1 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-xl transition-colors">
                {expanded
                  ? <><ChevronUp className="w-3 h-3" /> Show less</>
                  : <><ChevronDown className="w-3 h-3" /> Show all {entries.length} players</>
                }
              </button>
            )}
          </>
          ...
          >
            <div className="pointer-events-auto w-full sm:max-w-lg sm:mx-4 bg-slate-50 dark:bg-slate-900 sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden max-h-[92vh] sm:max-h-[88vh] flex flex-col">
                ...
                <div className="bg-white/15 rounded-2xl p-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-indigo-200 text-xs">
                        {xpState.level < 10
                          ? `${xpState.xpInCurrentLevel} / ${xpState.xpForNextLevel} XP to Level ${xpState.level + 1}`
                          : "Max Level Reached 🏆"}
                      </p>
                    </div>
                    <div className="ml-auto text-right">
              ...
              <div className="overflow-y-auto flex-1 p-4 space-y-4">
                ...
                {/* Overview stats grid */}
                <div className="grid grid-cols-2 gap-3">
                  <StatCard icon={<Globe className="w-4 h-4" />} label="Countries Found (Classic)"
                    value={classicStats.score} color="blue" />
                  <StatCard icon={<BookOpen className="w-4 h-4" />} label="Countries Studied"
                    value={totalStudied} sub={`of ${totalCountries} total`} color="amber" />
                  <StatCard icon={<Calendar className="w-4 h-4" />} label="Daily Streak"
                    value={`${daily_result?.streak ?? 0} days`}
                    sub={`Last score: ${daily_result?.score ?? 0}/10`} color="purple" />
                  <StatCard icon={<Award className="w-4 h-4" />} label="Continents Completed"
                    value={`${continentsCompleted}/6`} color="green" />
                </div>

                {/* ── Daily Leaderboard ── always visible for logged-in users */}
                {storedUser && (
                  <DailyLeaderboardSection username={storedUser.username} />
                )}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-md border border-slate-100 dark:border-slate-700">
                  ...
                  <ModeRow icon={<Timer className="w-3.5 h-3.5 text-green-600" />} label="60 seconds"
                    value={speedBest60 > 0 ? `${speedBest60} countries` : "Not played"}
                    accent="bg-green-100 dark:bg-green-900/30" />
                  <ModeRow icon={<Timer className="w-3.5 h-3.5 text-yellow-600" />} label="90 seconds"
                    value={speedBest90 > 0 ? `${speedBest90} countries` : "Not played"}
                    accent="bg-yellow-100 dark:bg-yellow-900/30" />
                  <ModeRow icon={<Timer className="w-3.5 h-3.5 text-orange-600" />} label="120 seconds"
                    value={speedBest120 > 0 ? `${speedBest120} countries` : "Not played"}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-md border border-slate-100 dark:border-slate-700">
                  ...
                  <ModeRow icon={<Map className="w-3.5 h-3.5 text-blue-600" />} label="Classic Mode"
                    value={classicStats.score > 0 ? `${classicStats.score} found` : "Not started"}
                    accent="bg-blue-100 dark:bg-blue-900/30" />
                  <ModeRow icon={<Calendar className="w-3.5 h-3.5 text-indigo-600" />} label="Daily Challenge"
                    value={daily_result && daily_result.score > 0 ? `Last: ${daily_result.score}/10` : "Not played today"}
                    accent="bg-indigo-100 dark:bg-indigo-900/30" />
                  <ModeRow icon={<BookOpen className="w-3.5 h-3.5 text-amber-600" />} label="Study Mode"
                    value={totalStudied > 0 ? `${totalStudied} studied` : "Not started"}
                    accent="bg-amber-100 dark:bg-amber-900/30" />
                  <ModeRow icon={<Zap className="w-3.5 h-3.5 text-yellow-600" />} label="Speed Round"
                    value={speedBest60 + speedBest90 + speedBest120 > 0 ? "Played ✓" : "Not played"}
                    accent="bg-yellow-100 dark:bg-yellow-900/30" />
                </div>

                {/* Continent Study Progress */}
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-md border border-slate-100 dark:border-slate-700">
                  <div className="flex items-center gap-2 mb-4">
                    <Globe className="w-4 h-4 text-teal-500" />
                    <p className="text-sm font-bold text-slate-900 dark:text-white">Continent Study Progress</p>
                  ...
                  {Object.entries(continentProgress).map(([name, { done, total }]) => (
                    <ContinentBar key={name} name={name} done={done} total={total}
                      gradient={continentGradients[name] ?? "from-slate-400 to-slate-500"} />
                  ))}
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-md border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {Array.from({ length: 10 }, (_, i) => i + 1).map(lvl => {
                      ...
                      const current  = xpState.level === lvl;
                      return (
                        <div key={lvl} className={`aspect-square rounded-xl flex items-center justify-center text-sm font-black transition-all ${
                          current ? "bg-gradient-to-br from-yellow-400 to-orange-500 text-white shadow-md scale-110"
                          : unlocked ? "bg-gradient-to-br from-indigo-400 to-purple-500 text-white"
                ...
                <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-4 text-center">
                  <p className="text-white font-bold text-sm mb-1">
                    {xpState.level < 10
                      ? `${xpState.xpForNextLevel - xpState.xpInCurrentLevel} XP until Level ${xpState.level + 1}`
                      : "You've reached the top! 🏆"}
                  </p>
                  <p className="text-indigo-200 text-xs">
                    {totalStudied < totalCountries
                      ? `${totalCountries - totalStudied} more countries to study across all continents`
                      : "All continents mastered — you're a Geography Legend!"}
                  </p>
                </div>
