...
//newest added (text input to click and submit)
// CapitalCityGame.tsx - Capital City Mode: See capital city name, find the country on the map
// CapitalCityGame.tsx - Capital City Mode: See capital city name, find the country on the map
...
export function CapitalCityGame({ onBackToMenu }: CapitalCityGameProps) {
  ...
  // Queue system state
  const QUEUE_STORAGE_KEY = "passQueue_capital-city";
  ...
  const loadCapitalRound = (countryName: string | null) => {
    ...
    setCurrentCapital(COUNTRY_CAPITALS[countryName]);
  };

  const bootstrapQueue = () => {
    const saved = loadQueueState(QUEUE_STORAGE_KEY);
    const initial = saved ?? initializeQueue(eligibleCountries, true);

    setQueueState(initial);
    setGameComplete(initial.queue.length === 0);

    const initialTarget = targetCountry ?? initial.queue[0]?.name ?? null;
    if (initialTarget) {
      loadCapitalRound(initialTarget);
    } else {
      setLocalTargetCountry(null);
      saveTargetCountry(null);
      setCurrentCapital("");
  ...
  useEffect(() => {
    ...
    if (targetCountry && COUNTRY_CAPITALS[targetCountry] && !correctCountries.includes(targetCountry)) {
      ...
      // Queue handles the next country
    }
  }, [countries, targetCountry, waitingForNextRound, gameComplete, queueState]);
  ...
  const handleConfirm = () => {
    ...
    transitionTimerRef.current = setTimeout(() => {
      ...
      loadCapitalRound(nextCountry.name);
    }, 2500);
  };
  ...
  const handlePass = () => {
    ...
    if (result.action === "defer") {
      ...
      const nextCountry = result.queueState.queue[0] ?? null;
      if (nextCountry) loadCapitalRound(nextCountry.name);
      return;
    }
  ...
  return (
    <div className="w-screen h-screen fixed inset-0 overflow-hidden bg-slate-100 dark:bg-slate-950">
      ...
      {/* Capital City Card + hide/show toggle */}
      {localTargetCountry && currentCapital && !gameComplete && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-1">
      ...
      <AnimatePresence>
        {config.isTV && (
          <ClassroomHUD
            score={score} totalCountries={eligibleCountries.length}
            selectedCountryName={selectedCountryName} allCountryNames={allCountryNames}
            onGuess={handleConfirm} onExitTVMode={toggleTVMode}
            onBackToMenu={onBackToMenu} onRestart={handleRestart}
            streak={currentStreak} targetLabel="Identify this country" mode="capital-city"
          />
        )}
