...
//Upper code works good.If there is error go back to up
//This works good.
// Globe3D.tsx - Bright version with beautiful star background
      ...
      {showButton && !isLoading && (
        <>
          ...
          >
            ...
            <p className="text-white/90 text-sm sm:text-lg md:text-xl drop-shadow-lg">
              Can you find every country on Earth?
            </p>
          </motion.div>

          <div className="absolute left-0 right-0 bottom-[calc(env(safe-area-inset-bottom,0px)+80px)] z-20 flex flex-col items-center px-4">
            <motion.button
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 1 }}
            ...
            <p className="mt-2 text-white/70 text-[10px] sm:text-xs md:text-sm text-center drop-shadow-lg">
              Drag the globe to explore the world
            </p>
          </div>
        </>
      )}
