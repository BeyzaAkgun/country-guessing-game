        ...
        <>
          <AnimatePresence>
            {selectedCountryName && (
              ...
              >
                <div
                  className="bg-slate-900/95 backdrop-blur-xl rounded-t-2xl p-4"
                  style={{
                    paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 16px)",
                  }}
                >
                  <div className="mx-auto w-full max-w-[460px] px-3 sm:px-0">
                    <StudyGuessPanel
                      selectedCountryName={selectedCountryName}
                      allCountryNames={allCountryNames}
                      onGuess={handleGuess}
                      onDismiss={() => setSelectedCountryName(null)}
                      accentGradient={colors.bg}
                    />
                  </div>
                </div>
              </motion.div>
...
function StudyGuessPanel({ selectedCountryName, allCountryNames, onGuess, onDismiss, accentGradient }: {
  ...
  const [showSuggestions, setShowSuggestions] = React.useState(false);
  const inputRef = React.useRef<HTMLInputElement>(null);
  React.useEffect(() => { setInput(""); setShowSuggestions(false); }, [selectedCountryName]);
