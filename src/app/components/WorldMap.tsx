...
//                     const isCorrect = correctCountries.includes(countryName);
//                     const isWrong = wrongCountries.includes(countryName);
//                     const isSkipped = skippedCountries.includes(countryName);
//                     const isSelected = selectedCountryName === countryName;
...
//           <div className="flex items-center gap-1.5">
//             <div className={`w-2.5 h-2.5 rounded-full ${isDarkMode ? "bg-rose-700" : "bg-rose-500"}`} />
//             <span className={isDarkMode ? "text-slate-300" : "text-slate-600"}>Wrong / Skipped</span>
//           </div>
//           <div className="flex items-center gap-1.5">
//             <div className={`w-2.5 h-2.5 rounded-full ${isDarkMode ? "bg-blue-600" : "bg-blue-500"}`} />
//             <span className={isDarkMode ? "text-slate-300" : "text-slate-600"}>Selected</span>
...
//hata olursa üste don.En son design hali.
//WorldMap.tsx
// import React, { memo, useEffect, useState, useRef } from "react";
// import {
//   ComposableMap,
...
//WorldMap.tsx
//Reset view button and leave button fix multiplayer mode
...
interface WorldMapProps {
  ...
  bottomOffset?: string;
  hideLegend?: boolean;
  /** When true, WorldMap will not render its own internal Reset View button.
   *  The parent is responsible for rendering a button and calling resetView() via ref.
   *  Defaults to false — existing call sites are unaffected. */
  hideResetButton?: boolean;
  /** Called whenever the zoom/pan view changes, reporting whether it's back at the default position. */
  onViewChange?: (isDefaultView: boolean) => void;
}
...
const WorldMap = forwardRef<WorldMapHandle, WorldMapProps>(({
  ...
  useEffect(() => {
    onViewChange?.(isDefaultView);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isDefaultView]);

  useImperativeHandle(ref, () => ({
    resetView: () => setPosition({ coordinates: [0, 0], zoom: 1 }),
