...
export function MultiplayerGame({ onBackToMenu, user, initialMatchId }: MultiplayerGameProps) {
  ...
  return (
    <div className="w-screen h-screen fixed inset-0 overflow-hidden">
      ...
      >
        <div className="grid w-full grid-cols-[1fr_minmax(0,24rem)_1fr] items-end gap-2">
          {/* Leave button */}
          <button
            onClick={() => {
              if (window.confirm("Leave match? You will forfeit.\n\nYou can reconnect within 15 seconds.")) {
            ...
            className="flex-shrink-0 flex items-center gap-1 px-2.5 py-2 bg-slate-900/80 hover:bg-slate-800 text-slate-400 hover:text-slate-200 rounded-xl text-xs font-semibold transition-all border border-slate-700/50 min-h-[44px] justify-self-start"
          >
            <WifiOff className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Leave</span>
          </button>
          ...
          {/* Submit panel */}
          <div className="w-full justify-self-center bg-slate-900/92 backdrop-blur-md rounded-xl sm:rounded-2xl border border-slate-700/50 shadow-xl p-2 sm:p-3">
            <AnimatePresence mode="wait">
              {lastResult && lastResult.correct ? (
                <motion.div
                  key="result-correct"
