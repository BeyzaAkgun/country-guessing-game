...
export function MultiplayerGame({ onBackToMenu, user, initialMatchId }: MultiplayerGameProps) {
  ...
  return (
    <div className="w-screen h-screen fixed inset-0 overflow-hidden">
      ...
      {/* ── Submit + Leave — fixed to bottom, respects safe area ── */}
      <div
        className="absolute inset-x-0 z-50 flex justify-center px-2 sm:px-3"
        style={{
          bottom: 0,
          paddingBottom: "max(env(safe-area-inset-bottom, 0px) + 80px, 88px)",
        }}
      >
        <div className="flex w-full max-w-sm items-end gap-2">
          ...
          {/* Submit panel — compact spacing */}
          <div className="min-w-0 flex-1 bg-slate-900/92 backdrop-blur-md rounded-xl sm:rounded-2xl border border-slate-700/50 shadow-xl p-2 sm:p-3">
            <AnimatePresence mode="wait">
              {lastResult && lastResult.correct ? (
                <motion.div key="result-correct" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                  className="text-center py-1 text-emerald-400">
