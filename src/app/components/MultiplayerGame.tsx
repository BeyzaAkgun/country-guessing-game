...
// //MOBILE FIXES.Sorun olursa bir önceki koda dön.....
// //WORRRKS
// //country-guessing-game-main/app/components/MultiplayerGame.tsx
// import React, { useState, useEffect, useRef, useCallback } from "react";
// import { motion, AnimatePresence } from "motion/react";
...
//   // ── Stored match check with forfeit pending check ─────────────────────────
//   useEffect(() => {
//     if (phase !== "lobby") {
//       setStoredMatchCheck({ loading: false, status: null, result: null });
//       return;
...
//     (async () => {
//       // First: check for a pending forfeit result (for the player who left)
//       try {
//         const forfeit = await matchmaking.checkPendingForfeit();
//         if (!cancelled && forfeit.has_pending && forfeit.result) {
//           setMatchEnd(forfeit.result);
//           setPhase("finished");
//           phaseRef.current = "finished";
//           clearActiveMatch();
//           setStoredMatchCheck({ loading: false, status: null, result: null });
...
//           if (user?.id && Array.isArray(forfeit.result.players)) {
//             const me = forfeit.result.players.find(p => p.user_id === user.id);
//             if (me && me.xp_earned > 0 && forfeit.match_id) {
...
//         }
//       } catch {
//         // Ignore forfeit check errors — not critical
//       }

//       // Then: check for an active stored match (existing logic)
//       const mid = getActiveMatch();
//       if (!mid) {
//         setStoredMatchCheck({ loading: false, status: null, result: null });
//         return;
...
//     if (mid) {
//       try {
//         const result = await matchmaking.forfeitMatch(mid);
//         // If we received match_result, show the loss modal immediately
//         if (result?.match_result) {
//           setMatchEnd(result.match_result);
//           setPhase("finished");
//           phaseRef.current = "finished";
//           // Award XP if not already awarded
//           const alreadyAwarded = xpAwardedRef.current === mid || sessionStorage.getItem("mp_xp_awarded_match") === mid;
...
//           setGraceCountdown(null);
//           if (!data.reconnect) {
//             posthog.capture("game_start", { mode: "multiplayer" });
//             setScores({});
//             setRoundResults([]);
//             setCorrectCountries([]);
//             setWrongCountries([]);
...
//           setMatchEnd({ winner_id: data.winner_id, is_draw: data.is_draw, forfeit: data.forfeit ?? false, players: data.players });
//           setPhase("finished");
//             const myResult = Array.isArray(data.players) ? data.players.find((p: any) => p.user_id === user?.id) : null;
//            const outcome = data.is_draw ? "draw" : data.winner_id === user?.id ? "win" : "loss";
//           //added for posthog analytics tracking
...
//           style={{ bottom: "max(env(safe-area-inset-bottom, 0px) + 160px, 168px)" }}
//         >
//           {roundResults.slice(-10).map((r, i) => (
//             <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }}
//               className={`w-4 h-4 sm:w-5 sm:h-5 rounded-md flex items-center justify-center text-[8px] sm:text-[9px] font-black ${r.myCorrect ? "bg-emerald-500/30 border border-emerald-500/50 text-emerald-400" : "bg-red-500/30 border border-red-500/50 text-red-400"}`}>
...
//Desktop UI reset view button and leave button fix
//country-guessing-game-main/app/components/MultiplayerGame.tsx
import React, { useState, useEffect, useRef, useCallback } from "react";
import { motion, AnimatePresence } from "motion/react";
import {
...
import { useCountryData } from "@/app/hooks/useCountryData";
import WorldMap, { type WorldMapHandle } from "@/app/components/WorldMap";
import type { StoredUser, MatchEndPlayer } from "@/api/client";
import { addXP } from "@/app/utils/xpSystem";
import { soundEffects } from "@/app/utils/soundEffects";
import { NewHintModal } from "./NewHintModal";
...
// Matches Tailwind's `sm` breakpoint, used elsewhere in this component for
// mobile/desktop split (e.g. "hidden sm:inline").
...
export function MultiplayerGame({ onBackToMenu, user, initialMatchId }: MultiplayerGameProps) {
  ...
  // Desktop-only: WorldMap renders its own Reset View button internally on
  // mobile (unchanged behavior). On desktop we hide WorldMap's internal
  // button and render our own in the bottom control row instead, so it
  // doesn't overlap the Level Badge / GameFAB in the bottom-right corner.
  const worldMapRef = useRef<WorldMapHandle>(null);
  const [isMapDefaultView, setIsMapDefaultView] = useState(true);
  ...
  return (
    <div className="w-screen h-screen fixed inset-0 overflow-hidden">
      ...
      {mapLoading ? (
        ...
        <div className="absolute inset-0 z-0">
          <WorldMap
            ...
            bottomOffset="9rem"
            hideLegend={true}
            // Desktop: we render our own Reset View button in the bottom
            // control row (see below), so suppress WorldMap's internal one
            // to avoid overlapping the Level Badge / GameFAB. Mobile keeps
            // WorldMap's default internal button, unchanged.
            hideResetButton={isDesktop}
            onViewChange={setIsMapDefaultView}
          />
        </div>
      )}
      ...
      {/*
        ── Bottom control row — Leave / Submit / Reset View ──
        Desktop (≥sm breakpoint): three-zone layout spanning the full width.
          Leave pinned to the far left, Submit panel centered (capped width),
          Reset View pinned to the far right — kept clear of the Level Badge /
          GameFAB which sits fixed bottom-right.
        Mobile: unchanged from before — Leave + Submit only, centered together
          as a single block. Reset View is not part of this row on mobile;
          WorldMap renders its own small internal button as it always did.
      */}
      <div
        className="absolute left-0 right-0 z-50 px-2 sm:px-3"
        style={{
      ...
      >
        <div className="flex items-end gap-2 w-full sm:max-w-none max-w-sm mx-auto">
          {/* Leave button — far left on desktop, left of submit on mobile */}
          <button
            onClick={() => {
              if (window.confirm("Leave match? You will forfeit.\n\nYou can reconnect within 15 seconds.")) {
                // clearActiveMatch(); cleanup(); onBackToMenu();
          ...
          {/* Submit panel — centered via mx-auto + capped width on desktop */}
          <div className="flex-1 bg-slate-900/92 backdrop-blur-md rounded-xl sm:rounded-2xl border border-slate-700/50 shadow-xl p-2 sm:p-3 sm:max-w-sm sm:mx-auto">
            <AnimatePresence mode="wait">
              {lastResult && lastResult.correct ? (
                <motion.div key="result-correct" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                  className="text-center py-1 text-emerald-400">
          ...
          {/* Reset View — desktop only, far right. Hidden entirely on mobile;
              WorldMap's own internal button covers mobile as before. */}
          {isDesktop && !isMapDefaultView && (
            <button
              onClick={handleResetView}
              className="hidden sm:flex flex-shrink-0 items-center gap-1.5 px-3 py-2 bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white rounded-xl text-xs font-semibold transition-all border border-slate-700/50 min-h-[44px]"
